...
module.exports = class IonVent extends Object {
    ...
    if (this.doesAlmStrtd) {
     ...
     // this.log("alarm--",data);
      //this.log(chalk.green.bold(`Data${data}`));
        mqttClient.publish(`ION/${this.ionId}/CURRENTALARM`, JSON.stringify(data), () => {
        });
  ...
  startTherapy(mqttClient) {
    ...
    if (this.doesAlmStrtd) {
      ...
      this.log(chalk.white.bold(`(${this.ionId})'s therapy already started.`));
      }else {     
       ...
       // this.log("alarm--",data);
        //this.log(chalk.green.bold(`Data${data}`));
          mqttClient.publish(`ION/${this.ionId}/HOMEMONITORING`, JSON.stringify(data), () => {
      ...
      this.log(chalk.white.bold(`No alarm running for (${this.ionId}). therapy can not be started.`));
    }
  }
  stopAlarm(clientmq) {
  ...
  startVentSetting(mqttClient) {
    ...
    if (this.doesAlmStrtd) {
      ...
      this.log(chalk.white.bold(`(${this.ionId})'s vent setting already running.`));
      }else {     
        this.log(chalk.yellow.bold(`(${this.ionId})'s vent setting  running now.`));
       ...
       // this.log("alarm--",data);
        //this.log(chalk.green.bold(`Data${data}`));
          mqttClient.publish(`ION/${this.ionId}/VENTSETTINGS`, JSON.stringify(data), () => {
          });
  ...
  startAlmSetting(mqttClient) {
    ...
    if (this.doesAlmStrtd) {
      ...
      this.log(chalk.white.bold(`(${this.ionId})'s alarm setting already running.`));
      }else {     
        this.log(chalk.yellow.bold(`(${this.ionId})'s alarm setting  running now.`));
       ...
       // this.log("alarm--",data);
        //this.log(chalk.green.bold(`Data${data}`));
          mqttClient.publish(`ION/${this.ionId}/ALARMSSETTING`, JSON.stringify(data), () => {
          });
          this.doesAlmSettingStarted = true;
        }, 6000);
      }
    }else{
      this.log(chalk.white.bold(`No alarm running for (${this.ionId}). No Alarm new alarm settings can be applied.`));
    }
  }
  ...
  stopTherapy(clientmq) {
   ...
   startMonitoringCurve(mqttClient,interval) {
    ...
    if (this.doesCurveStarted) {
      this.log(chalk.white.bold(`(${this.ionId})'s monitoring curve already started.`));
    } else {
      this.log(chalk.yellow.bold(`(${this.ionId})'s monitoring curve started now.`));
      this.intervalId = setInterval(() => {
        ...
        mqttClient.publish(`ION/${this.ionId}/CURVEMONITORING`, JSON.stringify(ventiltr), () => {
        });
  ...
  stopMonitoringCurve(clientmq) {
    ...
    clearInterval(this.intervalId);
    this.doesCurveStarted = false;
    this.log(chalk.red.bold(`[${this.ionId}]'s monitoring curve has been stopped.`));
  }
};
